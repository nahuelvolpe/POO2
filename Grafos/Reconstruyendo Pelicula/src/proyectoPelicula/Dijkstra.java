...
public class Dijkstra {
    ...
    public CaminoOptimo encontrarRutaMinimaDijkstra(int inicio, int fin) {
        ...
        if(!listos.contains(tmp)) {
            ...
            return new CaminoOptimo(0,0,null);
        }
        ...
        char[] rutaCaracteres = ruta.toCharArray();
        return new CaminoOptimo(distancia, rutaCaracteres.length, ruta);
    }
    ...
    public void encontrarRutaMinimaDijkstra(int inicio) {
        ...
        while(!cola.isEmpty()) {        // mientras que la cola no esta vacia
            ...
            for(int j=0; j<grafo[p].length; j++) {  // revisa los nodos hijos del nodo tmp
                if(grafo[p][j]==-1) continue;        // si no hay conexión no lo evalua
